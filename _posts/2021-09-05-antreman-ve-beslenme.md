---
layout: post
title: "Antreman Düzeni"
categories: Philosophy
---

# Fitness Başlangıç Antremanı

Ağırlıklı çalışma kısmı halter’le çalışmaya yeni başlayacaklara göre. Eğer stable barbell lift’lere aşinaysanız (Bench Press, Overhead Press, Row, Squat, Deadlift) daha kapsamlı bir rutin sizin için daha iyi olabilir.

Kilo artışı hedeflemiyorum aksine kilo vermeye çalışıyorum. Bu nedenle antreman süresince yediklerimi not alacak kilomu azaltmaya çalışacağım.

Bu rutin aslında bir alıştırma amaçlıyor. Yeni başlayanların spor salonuna girmesine, standart halter kaldırma ile antrenmana başlamasına ve sürekli spor salonuna gitme alışkanlığı kazanmasına yardımcı olacak basit, takip etmesi kolay bir rutin.

Zaman içerisinde tutarlı kalmaktaki yetersizlik ilerleme kaydetme yönünde en büyük eksiklik olduğundan amaç bariyeri aşağı çekerek başlama ve ve tutarlı kalmanın önündeki engeli mümkün olduğunca azaltmaya çalışılmış.

Bir diğer hedef ise, 1. günden itibaren birden fazla boyutta ilerlemeyi düşünerek başlamaktır – sadece ağırlığı artırmak değil, belirli bir ağırlıkta tekrarları artırmak. Aynı zamanda ağırlık çalışmasını HIIT ile desteklemek.

Bu rutinin 3 ay boyunca takip edilmesi tavsiye ediliyor. Bu noktadan sonra uzun vadeli ilerlemeye daha iyi hizmet etmesi açısından set sayılarının, yoğunluğun, hareket çeşitlerinin ve toplam hacmin yeniden düzenlendiği  daha kapsamlı bir rutine geçilmesi öneriliyor. GZCLP ve 5/3/1 Başlangıç bir sonraki adım için önerilmiş.

# Ağırlıklı Kuvvet Antremanı

Antemanımız Ağırlıklı Kuvvet Antremanı Barbell Strength Training olarak geçiyor. Çok basit bir şekilde Antreman A ve Antreman B olarak iki antreman düzenini sırayla uyguluyoruz. Her bir çalışmanın ertesi günü dinleniyoruz. Pazartesi/Çarşamba/Cuma gibi haftanın üç günü olabileceği gibi A -> Dinlen -> B -> Dinlen - > Tekrarla şeklinde devam edebiliyoruz. 

<table class="table">
<thead>
  <tr>
    <th>Antreman A</th>
    <th>Set 1</th>
    <th>Set 2</th>
    <th>Set 3</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td>Barbell Row</td>
    <td>5</td>
    <td>5</td>
    <td>5+</td>
  </tr>
  <tr>
    <td>Bench Press</td>
    <td>5</td>
    <td>5</td>
    <td>5+</td>
  </tr>
  <tr>
    <td>Squat (Ağırlıkla)</td>
    <td>5</td>
    <td>5</td>
    <td>5+</td>
  </tr>
</tbody>
</table>


<table class="table">
<thead>
  <tr>
    <th>Antreman B</th>
    <th>Set 1</th>
    <th>Set 2</th>
    <th>Set 3</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td>Chinup (veya benzeri)</td>
    <td>5</td>
    <td>5</td>
    <td>5+</td>
  </tr>
  <tr>
    <td>Overhead Press</td>
    <td>5</td>
    <td>5</td>
    <td>5+</td>
  </tr>
  <tr>
    <td>Deadlift</td>
    <td>5</td>
    <td>5</td>
    <td>5+</td>
  </tr>
</tbody>
</table>

Her bir hareketin tüm setlerini tamamladıktan sonra diğerine geçiyoruz. SetxTekrar şeklinde bir gösterim kullandık. + işaretinin anlamıysa son sette ne kadar yapabiliyorsan anlamındadır.

Son setin tam bir kas çökmesi (muscle failure) olmaması gerekir. 1-2 tane çok zorlasanız yapabilecek hissiniz kaldığı durumda bırakın. Ne zaman duracağınızı ölçmenin iyi bir yolu, formunuzun bozulduğu veya çubuğun hızının önceki tekrarlara kıyasla önemli ölçüde yavaşladığı zamandır.

Tekrarların net, kaliteli tekrarlar olmasını istiyoruz. Gerçek bir kas zorlanmasının nasıl bir his olduğunu anlama ve durma noktasının nasıl bulunacağını öğrenmek için zaman ayırmak bu rutinin amaçlardan biri.

Nasıl hissettiğinize bağlı olarak, her set arasında 2-3 dakika dinlenin. Egzersizler arasında 5 dakikaya kadar dinlenebilirsiniz.

### Ağırlık İlerlemesi:

Üst vücut ağırlık kaldırma (bench press, overhead press, barbell row, chinup) için her gün toplam ağırlığın 2.5 lbs arttırılması bekleniyor.

Squat ve deadlift için her gün toplam 5 Ibs arttırılması bekleniyor. 

Son setinizde 10'dan fazla tekrar yaparsanız, ağırlık ilerlemesini ikiye katlayın ve üsttekiler yerine 5/10 lbs ekleyin.

Bir hareket için toplamda en az 15 tekrarı tamamlayamazsanız, bir sonraki seferde ağırlıktan %10 çıkartın. Bunun yerine tekrarı arttırın.

Başlangıç Ağırlığının Bulunması : Boş ile başlanarak 10 kg (10-20 lbs) ile başlayıp parça parça arttırabilirsiniz.

Chinups Yapamıyorsanız. lat pulldown machine kullanabilirsiniz.

# Cardio ve Kondisyon

Ağırlık çalışmasına ek olarak dayanıklılığımızı arttırmak için ağırlık çalışmasına ek olarak haftada en az iki gün cardio ve kondisyon çalışması yapmak gerekiyor. 

Düşük Yoğunluklu Gün:  Seçenekleriniz tempolu yürüyüş, koşu, hafif bisiklet, eliptik makineler vb. Bu, bir seferde 15-30 dakika boyunca devam ettirebileceğimiz bir şey olmalı.

Yüksek Yoğunluklu Gün: Seçenek olarak HIIT, Tabata, Circuit training gibi seçenekler bulunmakta. Doğru bir şekilde yapıldığında 10-15 dakika fazlasıyla yeterli.

Rutinin bu kısmı çok, çok esnek, bu yüzden burada seçimlerimiz üzerinde fazla düşünmememiz tavsiye ediliyor. Yaptığımız şeyin özelliklerine fazla takılmadan sadece gerekli işi yaptığımızdan emin oluyoruz.

Kardiyo ve kondisyon çalışmalarınızı haftanın her günü yapılabiliyor. Bunu ağırlık kaldırdığınız gün yaparsanız, bunu kaldırmadan sonra yapmak daha iyidir - aksi takdirde sizi biraz yorar ve kaldırmadaki performansınız düşebilir.

### 8 Haftalık HIIT Programı

HIIT hight intersity interval training anlamına geliyor. HIIT aslında bir aralıklı antrenman (interval training) türü, aralarına dinlenme veya rahatlama periyotları serpiştirilmiş bir dizi yüksek yoğunluklu antrenmanı içeriyor. Burada amaç yüksek yoğunluklu periyotlarda artan eforu artırarak genel antrenman hacmini azaltmaya çalışır. .[1] 

Egzersiz anında performansı sınırlayacağı bu egzersizlerin aç karınla yapılmaması gerekiyor. Eğer açsanız çalışmadan 30-60 dakika önce bir protein içeceği veya BCAA öneriliyor.

Her antrenmandan önce kısa ısınma ve sonrasında soğumayı unutmayın. Direkt olarak sprint'e geçmeyin.

Setler sırasında hızınızı yalnızca biraz artırmayın. Tüm gücünüzü kullanarak mümkün olan en yüksek hıza çıkmaya çalışın.

İyileşme dönemlerinde yavaşlayın. Elinizden geldiği kadar toparlanmaya çalışın, böylece diğer setleri de yüksek yoğunlukla yapabilirsiniz.

Her antrenmanı haftada 2-4 kez gerçekleştirin. 2 ile başlayın, ardından kondisyonunuz ve toparlanmanız arttıkça 3-4'e çıkın.

Yükselmeden önce belirli bir aşamada iki haftadan fazla harcamanız gerekiyorsa bu şekilde yapın. Eğer 15 interval çok fazla geliyorsa 10 ile başlayıp 15’e çıkarmaya çalışın.

Bu koşullandırma rutinini çeşitli egzersizler kullanarak yapabilirsiniz. Bunlardan birini veya hoşunuza giden başka birini seçin ve arada sırada karıştırmaktan çekinmeyin.

Örneğin ben Cycling rutini ile başlıyorum. Koşu bandı da olabilir. Tabata orjinalinde 20 saniye açık 10 saniye kapalı biçiminde bunu 30 saniye açık 10 saniye kapalı biçimine çevirebiliriz.


<table class="table">
<thead>
  <tr>
    <th>Faz 1</th>
    <th>Set</th>
    <th>Süre</th>
    <th>Hareket</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Isınma</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>10</td>
    <td>15:1dk</td>
    <td>15 saniye sprint 1 dakika dinlenme</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Soğuma</td>
  </tr>
</tbody>
</table>


Isınma ve soğuma esnasında 5 dakika boyunca canlı ancak sürdürülebilir bir hızda sürüyoruz.

<table class="table">
<thead>
  <tr>
    <th>Faz 2</th>
    <th>Set</th>
    <th>Süre</th>
    <th>Hareket</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Isınma</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>10</td>
    <td>30:1dk</td>
    <td>15 saniye sprint 1 dakika dinlenme</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Soğuma</td>
  </tr>
</tbody>
</table>

<table class="table">
<thead>
  <tr>
    <th>Faz 3</th>
    <th>Set</th>
    <th>Süre</th>
    <th>Hareket</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Isınma</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>15</td>
    <td>30:30</td>
    <td>15 saniye sprint 1 dakika dinlenme</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Soğuma</td>
  </tr>
</tbody>
</table>

<table class="table">
<thead>
  <tr>
    <th scope="col">Faz 4</th>
    <th scope="col">Set</th>
    <th scope="col">Süre</th>
    <th scope="col">Hareket</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Isınma</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>15</td>
    <td>30:15</td>
    <td>15 saniye sprint 1 dakika dinlenme</td>
  </tr>
  <tr>
    <td>Cycle</td>
    <td>1</td>
    <td>5 dk</td>
    <td>Soğuma</td>
  </tr>
</tbody>
</table>

## Bakılacak:
* https://www.youtube.com/watch?v=edIK5SZYMZo&ab_channel=THENX
* Learning And Improving Lifts
* Beslenme Önerileri
* Back problems are very common. Yesterday I heard about a juijutsu coach on a podcast i watch his back is hurt. Geletinuous Disks deterioted to nothing. Bone on bone on his back basickly and he is in real pain. What can we do the cover that? Its very common with the office workers too. 
* Vitamin D
